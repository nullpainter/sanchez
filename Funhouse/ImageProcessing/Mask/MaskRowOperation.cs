...
namespace Funhouse.ImageProcessing.Mask
{
    ...
    /// </summary>
    public readonly struct MaskRowOperation : IRowOperation
    {
        private readonly Image<Rgba32> _source;
        private readonly bool _blurEdges;
        private readonly float _blurAmount;

        public MaskRowOperation(Image<Rgba32> source, bool blurEdges, float? blurAmount = null)
        {
            _source = source;
            _blurEdges = blurEdges;
        ...
        public void Invoke(int y)
        {
            ...
            for (var x = 0; x < span.Length; x++)
            {
                ...
                // Don't mask pixels inside Earth if we're not blurring, or within the blur amount if we are. The
                // blur is applied inside the Earth's radius and a distance of 1 indicates a point on the
                // circumference.
                if (distance < (_blurEdges ? _blurAmount : 1)) continue;
